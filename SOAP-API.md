> **Legacy documentation.** This file describes the SOAP API of the legacy StudentVUE app (Android `com.FreeLance.StudentVUE`, iOS `studentvue/id412050327`). Edupoint deprecated the student-data SOAP methods server-side, so most of what is documented below no longer works for the current app generation. The current **StudentVUE (New)** app (`com.edupoint.studentvue`) uses the [JSON API](JSON-API.md) instead — see the [README](README.md) for how the two fit together. The content below is preserved unchanged.
